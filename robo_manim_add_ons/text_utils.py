...
from manim import MathTex, VMobject, SurroundingRectangle, RED, ORANGE
...
class TextUtils:
    ...
    def part(scene, mathtext, *args):
        ...
        Args:
            scene: The scene object (required for part2 compatibility, not used here)
        ...
        Example:
            ...
            >>>
            >>> # Create from string
            >>> eq = TextUtils.part(self, "x^2 + y^2")
            >>>
            >>> # Extract from string with indices
            >>> p = TextUtils.part(self, "x^2 + y^2", 0)  # eq[0]
            >>> p2 = TextUtils.part(self, "x^2 + y^2", 1, 2)  # eq[1][2]
            >>>
            >>> # Extract from existing MathTex
            >>> existing_eq = MathTex("a + b")
            >>> p3 = TextUtils.part(self, existing_eq, 1)  # existing_eq[1]
            >>>
            >>> # Slice extraction
            >>> p4 = TextUtils.part(self, "x^2 + y", "1:3")  # eq[1:3]
            >>> p5 = TextUtils.part(self, existing_eq, 0, "1:4")  # existing_eq[0][1:4]
        """
        ...
        extracted = TextUtils._extract_part(mathtext_obj, *args)
        extracted.set_color(RED)
        return extracted

    @staticmethod
    def part2(scene, mathtext, *args):
        """
        ...
        Example:
            ...
            ...         p = TextUtils.part2(self, "x^2 + y^2", 0)
            ...         # Creates MathTex, extracts [0], colors RED, adds to scene with ORANGE box
            ...
            ...         # Debug: Show specific part of existing equation
            ...         eq = MathTex("a + b = c")
            ...         p2 = TextUtils.part2(self, eq, 2)
            ...         # Extracts eq[2], colors RED, adds to scene with ORANGE box
        """
        # Use part() to extract the part (already colored RED)
        extracted_part = TextUtils.part(scene, mathtext, *args)
...
def part(scene, mathtext, *args):
    """
    Standalone function for MathTex extraction. See TextUtils.part() for full documentation.

    Example:
        >>> from robo_manim_add_ons import part
        >>> p = part(self, "x^2 + y", 0)
        >>> p2 = part(self, existing_eq, 1, 2)
    """
    return TextUtils.part(scene, mathtext, *args)


def part2(scene, mathtext, *args):
    """
    Standalone function for debug MathTex extraction. See TextUtils.part2() for full documentation.

    Example:
        >>> from robo_manim_add_ons import part2
        >>> p = part2(self, "x^2 + y", 0)  # Shows with RED color and ORANGE box
    """
    return TextUtils.part2(scene, mathtext, *args)
