...
Demonstrations of `ill()` and `ilc()` functions.

**Short aliases:** `ill` (intersect_lines), `ilc` (intersect_line_circle)
...
```python
from robo_manim_add_ons import ill
...
intersection_dot = always_redraw(
    lambda: ill(fixed_line, rotating_line)
...
```python
from robo_manim_add_ons import ilc
