"""
Annotation Examples using dm (distance_marker)

This demonstrates how to use the dm function for
...
class LineObjectDemo(Scene):
    """Example: Using distance_marker with Line objects directly"""

    def construct(self):
        title = Text("Using Line Objects with Distance Markers", font_size=32).to_edge(UP)
        subtitle = Text("Pass Line/Arrow objects directly to dm()", font_size=20).next_to(title, DOWN)

        # Create various line objects
        ...
        marker1 = dm(line1, text="Line", color=BLUE, marker_offset=0.5)
        ...
        # Code examples
        code1 = Text('dm(line, text="Line")', font_size=14).next_to(line1, LEFT, buff=0.3)
        ...
        self.play(Create(line1), Write(code1))
        self.play(Create(marker1))
        self.wait(0.5)

        self.play(Create(line2), Write(code2))
        self.play(Create(marker2))
        self.wait(0.5)

        self.play(Create(line3), Write(code3))
        self.play(Create(marker3))
        self.wait(2)
